...
def drawLandmarks(im, landmarks, color=(0, 255, 0), radius=3):
    for i, p in enumerate(landmarks.parts()):
        ...
        cv2.putText(
            im, str(i), (p.x - 10, p.y - 10), font, 0.5, (255, 255, 255), 2, cv2.LINE_AA
        )
...
def mouthToJawRatio(landmarks):
    ...
    print(
        "lip to jaw ratio",
        (rightLipLandmark - leftLipLandmark) / (rightCheekLandmark - leftCheekLandmark),
    )
...
validateWithSingleImage()
# validateWithAllImages()
